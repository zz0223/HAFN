...
#       kernels = F.interpolate(input=kernels, size=(256*9, 64, 64), mode='nearest')
        ...
        kernellarge = F.interpolate(input=kernels, size=(256*25, data_with_est.shape[-1]//4, data_with_est.shape[-2]//4), mode='nearest')
        kernels = kernels.squeeze(dim=0)

        return kernels,conv1,conv2,conv3
class KPN2(nn.Module):
    def __init__(self, kernel_size=[3], sep_conv=False, channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        ...
        self.conv4 = Basic(256+256, 512, channel_att=False, spatial_att=False)
    ...
    def forward(self, x, conv1,conv2,conv3):

        conv4 = self.conv4(torch.cat([conv3, x], dim=1))
